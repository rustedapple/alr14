(function() {
	var Engine = window.Engine = {
		...
		SITE_URL: encodeURIComponent("http://adarkHome.doublespeakgames.com"),
		VERSION: 1.3,
		...
		init: function(options) {
			...
			$('<span>')
				.addClass('menuBtn')
				.text(_('github.'))
				.click(function() { window.open('https://github.com/doublespeakgames/adarkHome'); })
				.appendTo(menu);
		...
		getApp: function() {
			Events.startEvent({
				...
				scenes: {
					start: {
						text: [_('bring the Home with you.')],
						buttons: {
							'ios': {
								text: _('ios'),
								nextScene: 'end',
							...
							'android': {
								...
								nextScene: 'end',
								onChoose: function() {
									window.open('https://play.google.com/store/apps/details?id=com.yourcompany.adarkHome');
								}
							}
						}
					}
					...
					start: {
						...
						buttons: {
							...
							'twitter': {
								...
								nextScene: 'end',
								onChoose: function() {
									window.open('https://twitter.com/intent/tweet?text=A%20Dark%20Home&url=' + Engine.SITE_URL, 'sharer', 'width=660,height=260,location=no,menubar=no,resizable=no,scrollbars=yes,status=no,toolbar=no');
								}
							},
							'reddit': {
								text: _('reddit'),
				...
				switch(e.which) {
					...
					case 65:
						if(Engine.tabNavigation){
							...
							}else if(Engine.activeModule == Outside && Home.tab){
								Engine.activeModule.scrollSidebar('left', true);
								Engine.travelTo(Home);
							}
						}
					...
					case 68:
						if(Engine.tabNavigation){
							if(Engine.activeModule == Home && Outside.tab)
								Engine.travelTo(Outside);
